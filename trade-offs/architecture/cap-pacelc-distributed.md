...
> Вход — **шаг 2 NFR** ([consistency-as-nfr](../constraints/consistency-as-nfr.md)).  
> Выход — **паттерны Deep Dive §4.x** → **sync/async HLD §3.1** → **tech Deep Dive §4.x**.
...
```
шаг 2 NFR  NFR          →  «платёж = strong, лайк = eventual»
        ↓
Deep Dive §4.x  CAP/PACELC  →  «при partition: CP или AP; иначе: EL или EC»
        ↓
Deep Dive §4.x  Паттерны     →  sync/async repl, CQRS, Outbox, Saga, cache…
        ↓
HLD §3.1  API          →  sync RPC (платёж) vs async queue (email после заказа)
        ↓
Deep Dive §4.x  Технологии   →  PostgreSQL, Cassandra, Kafka…
```
...
| CAP/PACELC | Типичные паттерны (Deep Dive §4.x) | Sync vs Async (HLD §3.1) | Технологии (Deep Dive §4.x) |
|------------|---------------------------|-------------------------|---------------------|
...
| Тип задачи | шаг 2 NFR | CAP/PACELC | Паттерны | API | Технологии |
|------------|-------------|------------|----------|-----|------------|
